...
/**
 * Apply key mapping to an object
 * 
 * Maps source keys to target keys with optional transformations.
 * Supports:
 * - Simple key rename (string target)
 ...
 * - Default values
 */
export function applyKeyMap(obj: any, keyMap: KeyMap, context: FlowContext): any {
  ...
  for (const [sourceKey, targetConfig] of Object.entries(keyMap)) {
    if (sourceKey.includes('*')) {
      ...
      mapWildcard(obj, sourceKey, targetConfig, result, context, mappedKeys);
    } else {
      // Handle simple/nested key mapping
      mapSingleKey(obj, sourceKey, targetConfig, result, context, mappedKeys);
    }
  }

  // Copy unmapped keys (preserve keys not in the map)
  copyUnmappedKeys(obj, result, mappedKeys);
...
/**
 * Validate key map configuration
 * 
 * Checks for:
 * - Invalid patterns
 ...
 * - Unknown transforms
 */
export function validateKeyMap(keyMap: KeyMap): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  for (const [sourceKey, targetConfig] of Object.entries(keyMap)) {
    // Validate source key
    if (!sourceKey || sourceKey.trim() === '') {
      errors.push('Empty source key in key map');
      continue;
    }

    // Validate target config
    if (typeof targetConfig === 'string') {
      if (!targetConfig || targetConfig.trim() === '') {
        errors.push(`Empty target for source key: ${sourceKey}`);
      }
    } else if (typeof targetConfig === 'object') {
      // Validate KeyMapConfig
      if (!targetConfig.to || targetConfig.to.trim() === '') {
        errors.push(`Missing or empty 'to' field for source key: ${sourceKey}`);
      }

      // Validate transform references
      if (targetConfig.transform) {
        const transforms = Array.isArray(targetConfig.transform) 
          ? targetConfig.transform 
          : [targetConfig.transform];

        for (const transform of transforms) {
          if (!defaultTransformRegistry.has(transform)) {
            errors.push(
              `Unknown transform '${transform}' for key ${sourceKey}. ` +
              `Available: ${defaultTransformRegistry.list().join(', ')}`
            );
          }
        }
      }

      // Validate values lookup table
      if (targetConfig.values && typeof targetConfig.values !== 'object') {
        errors.push(`Invalid 'values' field for key ${sourceKey}: must be an object`);
      }
    } else {
      errors.push(`Invalid target config type for key ${sourceKey}: must be string or object`);
    }

    // Validate wildcard patterns
    const wildcardCount = (sourceKey.match(/\*/g) || []).length;
    if (wildcardCount > 1) {
      errors.push(`Multiple wildcards not supported in key pattern: ${sourceKey}`);
    }

    if (wildcardCount === 1) {
      const target = typeof targetConfig === 'string' ? targetConfig : targetConfig.to;
      const targetWildcardCount = (target.match(/\*/g) || []).length;
      
      if (targetWildcardCount !== 1) {
        errors.push(
          `Wildcard pattern mismatch: source '${sourceKey}' has wildcard, ` +
          `but target '${target}' ${targetWildcardCount === 0 ? 'has none' : 'has multiple'}`
        );
      }
    }
  }
