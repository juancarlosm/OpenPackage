 ...
 * Reads all files from the directory and loads package.yml.
 */
export async function loadPackageFromDirectory(dirPath: string): Promise<Package> {
  ...
  if (!config) {
    throw new ValidationError(
      ...
      `Missing .openpackage/package.yml`
    );
  }
...
export async function loadPackageFromTarball(tarballPath: string): Promise<Package> {
  ...
  // Find package.yml in extracted files
  const packageYmlFile = extracted.files.find(
    f => f.path === PACKAGE_PATHS.MANIFEST_RELATIVE || f.path === 'package.yml'
  );
  
  if (!packageYmlFile) {
    throw new ValidationError(
      `Tarball '${tarballPath}' does not contain a valid package.yml file`
    );
  }
  
  // Parse package.yml content
  const yaml = await import('js-yaml');
  const config = yaml.load(packageYmlFile.content) as PackageYml;
  
  if (!config.name) {
    throw new ValidationError(
      `Tarball '${tarballPath}' contains invalid package.yml: missing name field`
    );
  }
