...
- **Usage:** `opkg save` (package at effective cwd via shell or `--cwd <dir>`; see [../cli-options.md]), `opkg save <package>`, or `opkg save <package> <path>` (runs add for the path, then saves).
